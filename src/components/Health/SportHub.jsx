...
export default function SportHub() {
  ...
  const handleSaveWorkout = (newOrUpdatedWorkout) => {
    if (selectedWorkout) {
      setWorkouts(workouts.map(w => w.id === newOrUpdatedWorkout.id ? newOrUpdatedWorkout : w));
    } else {
      setWorkouts([newOrUpdatedWorkout, ...workouts].slice(0, 50)); // Keep last 50
    }
  };
